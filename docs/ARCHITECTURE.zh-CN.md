...
MP4 直链下载使用 HTTP Range 请求进行多线程并行下载单个文件。
...
探测服务器支持：
    │
    ├── 1. HEAD 请求 → 检查 Accept-Ranges 头
    ├── 2. GET 请求 → HEAD 失败时用 GET 探测
    └── 3. Range 测试 → 发送 Range: bytes=0-0，看是否返回 206
         │
...
续传流程：
    │
    ├── 检查 _mp4_downloaded（已保存的进度）
    ├── 检查 .tmp 文件是否存在且大小匹配
    ├── 匹配 → Range: bytes=<downloaded>-（继续下载）
    └── 不匹配 → 从头开始下载
```
