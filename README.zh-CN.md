...
| **下载前分析** | 预解析 M3U8 显示清晰度、码率、时长、分片数、加密状态 |
| **批量下载** | 支持 TXT 文件导入和手动粘贴多个链接，一键创建多个任务 |
...
| **文件完整性验证** | 下载完成后用 ffprobe 检查时长、分辨率、视频/音频编码 |
...
- ✨ **剩余时间显示** — M3U8 和 MP4 下载均显示预估剩余时间（HH:MM:SS）
- ✨ **自动更新检查** — 可选功能，在 config.json 中设置 `"auto_update_check": true` 开启

### v0.26

- ✨ **批量下载** — 支持 TXT 文件导入和手动粘贴多个链接，自动从 URL 提取文件名
- ✨ **下载前分析** — 预解析 M3U8 显示清晰度、码率、时长、分片数、加密状态

### v0.25

- ✨ **HLS 兼容增强** — 解析 `EXT-X-ENDLIST`（VOD 检测）、`EXT-X-DISCONTINUITY`（日志记录）、`EXT-X-PROGRAM-DATE-TIME`
- ✨ **直播流检测** — 无 `EXT-X-ENDLIST` 的流标记为直播并输出警告
- 🔧 **IV 解析健壮性** — 支持无 `0x` 前缀、冒号分隔、空格；不足 16 字节自动补零
- 🔧 **解密边界处理** — 非 16 字节对齐的加密数据自动截断并警告

### v0.24

- ✨ **文件完整性验证** — 下载完成后用 ffprobe 检查时长、分辨率、视频/音频编码，结果显示在任务卡片
- ✨ **GitHub Actions 自动构建** — 推送 `v*` 标签自动构建 Windows exe 并发布 Release（含 SHA256 校验）
